...
while (($#)); do
  case "$1" in
    ...
    -h|--help)
      echo "Usage: sudo install-vps.sh --public-key-b64 BASE64 [--tunnel-user USER] [--public-ssh-port PORT] [--remote-port PORT]"
      exit 0 ;;
...
Match User $TUNNEL_USER
    ...
    PermitTTY no
    PermitListen localhost:$REMOTE_PORT
    MaxSessions 0
